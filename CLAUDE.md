...
**Bridgic Browser** is an LLM-driven browser automation library built on Playwright with built-in stealth mode. It provides 68+ browser tools organized into categories, an accessibility tree-based snapshot system, a stable element reference system (`e1`, `e2`, …) designed for reliable AI agent interactions, and a `bridgic-browser` CLI tool backed by a persistent daemon.
...
└── cli/              # CLI tool (bridgic-browser command)
    ...
    ├── _commands.py      # Click command definitions (25 commands, SectionedGroup)
...
**`ToolPreset` sizes**: MINIMAL (10), NAVIGATION (4), SCRAPING (13), FORM_FILLING (20), TESTING (28), INTERACTIVE (40), DEVELOPER (18), COMPLETE (68).
...
bridgic-browser click @e2
 ...
 send_command("click",...)   /tmp/bridgic-browser.sock    asyncio server
