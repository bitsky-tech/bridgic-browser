...
CLI_COMMAND_META: dict[str, tuple[ToolCategory, str]] = {
    ...
    "snapshot": (ToolCategory.SNAPSHOT, "Get an accessibility tree representation of the current page with refs (like e1, e2) [-i] [-f|-F] [-s OFFSET]"),
    ...
    "wait": (ToolCategory.WAIT, "Wait for SECONDS or until TEXT appears or disappears [--gone]"),
    ...
    "eval-on": (ToolCategory.EVALUATE, "Evaluate JavaScript with a REF element as argument"),
    ...
    "close": (ToolCategory.LIFECYCLE, "Close/Stop the browser session"),
