...
- To connect to an existing Chrome via CDP, pass `--cdp` to `bridgic-browser open`, or set the `BRIDGIC_CDP` env var. The `--cdp` flag accepts a port number, `ws://`/`wss://` URL, `http://host:port`, or `auto`.
...
| `cdp_url` | string (ws:// URL) | Connect to existing Chrome via CDP instead of launching. Set via `BRIDGIC_CDP` env var or `--cdp` CLI flag (not via config JSON). |
